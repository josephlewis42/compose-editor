...
1. `protoc-gen-go-lite` - regenerates the Go types under
   `pkg/proto/composeeditor/v1/*.pb.go`. This plugin isn't installed
   globally; the Makefile builds it on demand from the `go.mod` `tool`
   dependency (`github.com/aperturerobotics/protobuf-go-lite/cmd/protoc-gen-go-lite`)
   into `build/protoc-gen-go-lite`.
...
with source info to `build/proto-image.binpb` (via `--descriptor_set_out
